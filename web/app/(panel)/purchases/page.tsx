    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-dashed border-border bg-surface p-12 text-center">
          ...
          <p className="mt-3 text-sm font-medium text-muted-foreground">No purchases yet</p>
          <p className="mt-1 text-[12px] text-faint">Assign a plan to a user to get started</p>
          ...
          <div className="hidden lg:block rounded-xl border border-border bg-surface">
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-border">
                {purchases.map((p) => (
                  <tr key={p.id} className="text-muted-foreground transition-colors hover:bg-surface-2">
                    <td className="px-5 py-3.5">
                      <div>
                        <span className="font-medium text-foreground">{p.user?.username || "—"}</span>
                        <p className="text-[11px] text-faint">{p.user?.email}</p>
                      </div>
                    </td>
                    <td className="px-5 py-3.5">
                      <span className="text-[13px] text-foreground">{p.plan?.name || "—"}</span>
                      {p.note && (
                        <p className="mt-0.5 text-[11px] text-faint">{p.note}</p>
                      )}
                    </td>
                    ...
                    <td className="px-5 py-3.5 text-[12px] text-muted-foreground">{formatDate(p.purchasedAt)}</td>
                    <td className="px-5 py-3.5 text-[12px] text-muted-foreground">{formatDate(p.expiresAt)}</td>
                ...
                <div className="p-4 space-y-2">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <h3 className="font-medium text-foreground truncate">{p.user?.username || "—"}</h3>
                      <p className="text-[11px] text-faint mt-0.5">{p.user?.email}</p>
                    </div>
                    <Badge variant={statusVariant(p.status)} className="shrink-0">{p.status}</Badge>
                  </div>
                  <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[12px]">
                    <span className="text-foreground">{p.plan?.name}</span>
                    <span className="text-muted-foreground">₹{p.amount}</span>
                    <span className="text-faint">{formatDate(p.purchasedAt)} → {formatDate(p.expiresAt)}</span>
                  </div>
                  {p.note && (
                    <p className="text-[11px] text-faint mt-1">{p.note}</p>
                  )}
                </div>
                {p.status === "active" && (
                  <div className="border-t border-border">
                    <button
                      onClick={() => cancelPurchase(p.id)}
                      className="w-full flex items-center justify-center gap-1.5 py-2.5 text-xs font-medium text-red-600 dark:text-red-400 transition-colors hover:bg-red-500/10"
                    >
